...
document.addEventListener( 'DOMContentLoaded', function( event ) {
    ...
    function makeModel(jsonPath, callback){
        ...
        */
        ...
        function addFunctionalButtons(){
            ...
            addFunctionalButton(10000, '<div class="icon"><img src="img/icons/back.png"></div>назад', 'back');
            addFunctionalButton(10003, '<div class="icon"><img src="img/icons/back.png"></div>в&nbsp;начало', 'backBegin');
            addFunctionalButton(10001, '<div class="icon"><img src="img/icons/menu-2.png"></div>меню', 'menu');
    ...
    function makeView(model){
                    ...
                    switch(force){
                        ...
                        case 'slideForce':
                            if(d.active){
                                ...
                                return (width/5 + width/3*(nodeDepth - activeDepth)) - width/1.4;
                            }
                            break;
                        //мощность силы которая задаеть горизонтальную координату
                        case 'slideForceStr':
                            return d.active ? 0.5 : 0.65;
                            break;
                        //сила задает вертикальную координату для каждой ноды
                        case 'verticalForce':
                            if(d.active){
        ...
        function simulationResize(resizeEvent){
            ...
            simulation.force("slideForce", d3.forceX(self.slideForce).strength(self.slideForceStr));
            simulation.force("verticalForce", d3.forceY(self.verticalForce).strength(self.verticalForceStr));
            simulation.force("radial", d3.forceRadial(self.radial).strength(self.radialStr).x(self.radialX()).y(self.radialY()) );
            simulation.force("collide", self.isolateForce(d3.forceCollide().radius(self.getColideRadius).strength(self.colideRadiusStr()).iterations(self.getColideRadiusIterations()),self.collideForceIsolate));
            simulation.force("order", d3.forceY(self.orderForce).strength(self.orderForceStr));
