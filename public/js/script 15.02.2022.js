...
document.addEventListener( 'DOMContentLoaded', function( event ) {
        ...
        d3newNodes // работа с новыми узлами (выполняеться для каждого узла в отдельности)
          ...
          .on('start', function repeat() {
            ...
            if (numsOfChildren >= 4 && currentNodeID === lastNodeID) {
              ...
              // console.log('+++++');
              bodyClass.classList.add('v-wheel');
            }
            if (numsOfChildren < 4 && currentNodeID === lastNodeID) {
              // иначе (если узлов < 4)
              //! ***
              bodyClass.classList.remove('v-wheel');
    ...
    function makeView(model){
        ...
        function radialX(d) {
          let radialXDelta =  width / 2; // for mobile коеффициент смещения окр-ти притяжения по оси X //! ***
          // let radialXDelta = width / 4; // for mobile коеффициент смещения окр-ти притяжения по оси X //! ***
          // let radialXDelta = width / 4; // коеффициент смещения окр-ти притяжения по оси X //! ***
          if (!verticalScreen) {
            //! ***
            // for desktop
            radialXDelta = width / 6;
          }
        ...
        function radialY(d) {
          //! ***
          let radialYDelta = 0; // коеффициент смещения окр-ти притяжения по оси Y //! ***
          // let radialYDelta = (height / 2) * 0.25; // коеффициент смещения окр-ти притяжения по оси Y //! ***
          if (!verticalScreen) {
            //! ***
                    ...
                    switch(force){
                        ...
                        case 'slideForce':
                            if((d.active) && goOut){
                                ...
                                //   coords = (width/2 + width/2*(nodeDepth - activeDepth)) - width/0.7;
                                  coords = - width * 2;
                            }else if(d.active){
                                if(childrens.length >= 4) {
                                  coords = (width/2 + width/2*(nodeDepth - activeDepth)) - width/1.2;
                                }else{
                                  coords = (width/2 + width/2*(nodeDepth - activeDepth)) - width/1.2;
                                }
                            }else{
                                if(childrens.length >= 4) {
                                  coords = width * 0.25;
                                  // coords = (width/2 + width/2*(nodeDepth - activeDepth)) - width/0.9;
                                }else{
                                  // return (width/5 + width/2*(nodeDepth - activeDepth)) - width/2;
                                  coords = (width/5 + width/3*(nodeDepth - activeDepth)) - width/1.35;
                                }
                            }
                        ...
                        case 'verticalForce':
                            ...
                            if((d.active) && goOut){
                              //! ***
                              coords = 0; //! ***
                              // coords = height / 18 + ((height * 4) / 5) * (nodeDepth - activeDepth) - height / 7; //! ***
                              // coords = height / 18 + ((height * 4) / 5) * (nodeDepth - activeDepth) - height / 2.75; //! ***
                              // draw.axis(1000, coords, -1000, coords, d.id, 'HorizontalAxis'); // отрисовка оси
                              // return coords;
                            } else if ((d.active) && liveCard) {
                              //! ***
                              coords = height / 18 + ((height * 4) / 5) * (nodeDepth - activeDepth) - height / 2.75; //! ***
                              // draw.axis(1000, coords, -1000, coords, d.id, 'HorizontalAxis'); // отрисовка оси
                              // return coords;
                            ...
                            //     coords = height / 18 + ((height * 4) / 5) * (nodeDepth - activeDepth) - height / 1.3;
                            //   }
                            // }else if(d.liveCardS){
                            //   if(d.active){
                            //     // liveCardS active
                            ...
                            //     }else{
                            //         // children with liveCard only
                            //         return (height/18 + (height*4/5)*(nodeDepth - activeDepth)) - height/0.5;
                            //     }
                            // }else if(d.liveCardS){
                            //     if(d.active){
                            //         // liveCardS active
                            //         return (height/18 + (height*4/5)*(nodeDepth - activeDepth)) - height/2.75;
                            //     }else{
                            //         // liveCardS not active 
                            //         return (height/18 + (height*4/5)*(nodeDepth - activeDepth)) - height/1.3;
                            //     }
                            // }else{
                            //     if(d.active){
                            //         // active liveCardS = false and liveCard = false
                        ...
                        case 'verticalForceStr':
                            return d.active ? 0.35 : 0.25;
                            // return d.active ? 0.35 : 0.05;
                            break;
                        //радиус радиальной силы
                        case 'radial': //! ***
                          let radius; //! ***
                          let radialRadCoef = 0.49; //! ***
                          radius = width; //! ***
                          console.log(radius);
                          draw.radial(radialXCoord, radialYCoord, radius, d.id, 'radial'); // отрисовка окружности притяжения //! ***
                          return radius; //! ***
                        ...
                        case 'getColideRadius':
                            ...
                            // if(childrens.includes(d.id)){
                            //      if(childrens.length > 2) {
                            //       let collRadCoef = 375/65
                            //       return width/collRadCoef;
                            //       console.log(collRadCoef);
                            //     }else{
                        ...
                        case 'orderForce':
                            if (childrens.includes(d.id)) {
                              ...
                              if (order.length <= 3) {
                                ...
                                onePart = (height / 4) * 0.7; //! ***
                              }

                              if(childrens.length >= 4) {
                                for (let i = 0; i < order.length; i++) {
                                  if (order[i].id == d.id) {
                                    const coords = (onePart * (i + 1) - onePart + mobileDelta) * 2.15; // изначально mobileDelta=0 и меняется только при свайпе //! ***
                                ...
                                for (let i = 0; i < order.length; i++) {
                                  if (order[i].id == d.id) {
                                    const coords = (onePart * (i + 1) - onePart + mobileDelta) * 0.85; // изначально mobileDelta=0 и меняется только при свайпе //! ***
                                    // draw.axis(1000, coords, -1000, coords, d.id, 'orderForce'); // отрисовка оси //! ***
                                    return coords; //! ***
                                  }
                                }
                            ...
                            //     for (let i = 0; i < order.length; i++) {
                            //       if (order[i].id == d.id) {
                            //         const coords = (onePart * (i + 1) - onePart + mobileDelta) * 1.5; // изначально mobileDelta=0 и меняется только при свайпе //! ***
                            //         // draw.axis(1000, coords, -1000, coords, d.id, 'orderForce'); // отрисовка оси //! ***
                            //         return coords; //! ***
                            //       }
                            //     }
                        ...
                        case 'orderForce':
                            ...
                            if (childrens.includes(d.id)) {
                              ...
                              if (order.length <= 3) {
                                ...
                                // если кол-во дочерних узлов > 5, расстояние между осями фиксированное
